...
tmux_attach="N"
test_only="False"
# Now weights resume the training, models, optmizers, etc... instead of just reloading the model weights
weights="''"
rerank="False"
wandb_id="''"
mode="''"
while getopts c:g:s:n:a:t:w:r:i:m: flag
do
    case "${flag}" in
...
if [ $? != 0 ]; then
    ...
    if [ $new_session = "Y" ]; then
        ...
        tmux send-keys -t $tmux_session "CUBLAS_WORKSPACE_CONFIG=:16:8 CUDA_VISIBLE_DEVICES=$gpu_id python scripts/main.py --config-file $configuration --run_id $wandb_id --mode $mode test.evaluate $test_only model.resume $weights test.rerank $rerank" Enter
