...
class Dataset(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.val is True:
            if len(val_query[0]) == 3:
                val_query = [(*items, 0) for items in val_query]
            if len(val_gallery[0]) == 3:
                val_gallery = [(*items, 0) for items in val_gallery]
        ...
        if self.mode == 'train':
            ...
            if self.val is True:
                ...
                raise ValueError(
                    'Invalid mode. Got val={}, but expected to be True'
                    'one of [val_query | val_gallery] is selected'.format(self.val)
                )
            ...
            raise ValueError(
                ...
                'one of [train | query | gallery | val_query | val_gallery]'.format(self.mode)
            )
    ...
    def __repr__(self):
        ...
        if self.val is True:
            num_val_query_pids = self.get_num_pids(self.val_query)
            num_val_query_cams = self.get_num_cams(self.val_query)
            num_val_gallery_pids = self.get_num_pids(self.val_gallery)
            num_val_gallery_cams = self.get_num_cams(self.val_gallery)
            ...
            msg = '  ----------------------------------------\n' \
                ...
                '  train      | {:5d} | {:7d} | {:9d}\n' \
                '  val_query  | {:5d} | {:7d} | {:9d}\n' \
                '  val_gallery| {:5d} | {:7d} | {:9d}\n' \
                '  query      | {:5d} | {:7d} | {:9d}\n' \
                '  gallery    | {:5d} | {:7d} | {:9d}\n' \
                '  ----------------------------------------\n' \
                '  items: images/tracklets for image/video dataset\n'.format(
                    num_train_pids, len(self.train), num_train_cams,
                    num_val_query_pids, len(self.val_query), num_val_query_cams,
                    num_val_gallery_pids, len(self.val_gallery), num_val_gallery_cams,
...
class ImageDataset(Dataset):
    ...
    def __init__(self, train, query, gallery, val=False, val_query=None, val_gallery=None, **kwargs):
        if val is True:
            super(ImageDataset, self).__init__(train, query, gallery, val, val_query, val_gallery, **kwargs)
        else:
    ...
    def show_summary(self):
        ...
        if self.val is True:
            ...
            print(
                '  val_query  | {:5d} | {:8d} | {:9d}'.format(
                    num_val_query_pids, len(self.val_query), num_val_query_cams
                )
            )
            print(
                '  val_gallery| {:5d} | {:8d} | {:9d}'.format(
                    num_val_gallery_pids, len(self.val_gallery), num_val_gallery_cams
                )
            )
